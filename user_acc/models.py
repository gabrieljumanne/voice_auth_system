...
class CustomUser(AbstractBaseUser, PermissionsMixin):
    ...
    def clean(self):
        ...
        if self.date_of_birth:
            ...
            today = date.today()
            age = today.year - self.date_of_birth.year -((today.month, today.date) < (self.date_of_birth.month, self.date_of_birth.date))
            if age < 13:
                raise ValidationError(_("You must be above 13 years old to register .."))
    ...
    def set_theme_preference(self, theme):
        ...
        if theme in ["light", "dark", "system"]:
            ...
            self.save(update_fields=["theme_preferences"])
